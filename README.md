...
**LC1220：用元音字母组成字符串，有相邻约束，求方案数量**

**思路**：一个合法字符串就是一系列每个位置填什么的决策，求总数，dp解决。f[i][j]代表所有长度为i，结尾为j的合法字符串的数量。根据这个字符串倒数第二位是什么来分类。

**总结心得/问题模型**：
1. 答案是决策序列，求可能的方案数 -> dp

**LC1223：骰子1-6，加限制不能连续几次i，问所有可能的长度为n的序列有多少个**

**思路**：答案是骰子结果的序列，问有多少种可能结果，dp解决。f[i][j][k]代表所有长度为i，结尾为j，j在最后连续出现k次的所有序列的数量。枚举长度为i-1时，所有可能的结尾和结尾长度。

**总结心得/问题模型**：
1. 答案是决策序列，求可能的方案数 -> dp
2. 求方案数的dp，初始化只用考虑合法的最小的就可以，很清楚

**LC312：一排气球，打破一个获得的分数是左边x自己x右边，问打完最大的分数是多少**

**思路**：一系列决策是打哪个气球，求最大分数，识别出dp。然后这种一排东西，相邻的东西在变化，识别出区间dp。所以f[i][j]代表把[i+1:j-1]内气球打完的所有方案的最大分数。枚举最后一个打掉的气球位置k=[i+1:j-1]。

**总结心得/问题模型**：
1. 答案是决策序列，求最大value -> dp
2. value和相邻的东西有关且相邻的会变 -> 区间dp
